					...
					if(is_object ( $prow ))
					{
						...
						foreach($prow as $k => $f)  // looping through the fields of the row
						{
							...
							{	// we skip the row id which is in position 0
								...
								{
									$linkToDetail = JRoute::_('index.php?option=com_'._cppl_this_com_name.'&view=easytablerecord&id='.$this->tableId.':'.$this->easytable->easytablealias.'&rid='.$rowId.'&rllabel='.JFilterOutput::stringURLSafe(substr($f, 0,100)));
