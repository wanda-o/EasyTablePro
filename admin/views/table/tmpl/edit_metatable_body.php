...
<?php
	...
	foreach ($this->item->table_meta as $metaRow)
	{
		...
		if($this->item->etet)
		{
			echo '<input type="hidden" name="origfieldalias'.$mRId.'" value="'.$metaRow['fieldalias'].'" /><input type="hidden" name="fieldalias'.$mRId.'" value="'.$metaRow['fieldalias'].'" />'.$metaRow['fieldalias'];
		}
		else
		{
		...
		echo('<td>'.$this->getTypeList($mRId, $metaRow['type']).'<br />'.
			'<input type="hidden" name="origfieldtype'.$mRId.'" value="'.$metaRow['type'].'" />'.
			'<input type="text" value="'.$this->getFieldOptions($metaRow['params']).'" name="fieldoptions'.$mRId.'" class="hasTip" title="'.JText::_( 'COM_EASYTABLEPRO_TABLE_FIELDSET_COL_OPTIONS_TT' ).'" /></td>');			// Type / Field Options
